...
def main():
    ...
    # Load new Haifa data
    with open('haifa_shelters.json', 'r', encoding='utf-8') as f:
        haifa_shelters = json.load(f)
    
    print(f"{'='*60}")
    print(f"HAIFA SHELTERS IMPORT")
    print(f"{'='*60}")
    print(f"Loaded {len(haifa_shelters)} Haifa shelters from JSON\n")
    
    # Get existing shelters in Haifa area (approx bounds)
    # Haifa: lat 32.7-32.85, lon 34.95-35.1
    existing_shelters = list(shelters_collection.find({
        'location.coordinates.0': {'$gte': 34.95, '$lte': 35.1},
        'location.coordinates.1': {'$gte': 32.7, '$lte': 32.85}
    }))
    ...
    for new_shelter in haifa_shelters:
        ...
        if duplicate:
            ...
            update_data['source'] = 'haifa_kml_2024'
